original=`Afghanistan,AF,AFG,004,ISO 3166-2:AF,Asia,Southern Asia,"",142,034,""
...
layer1 = [...original.matchAll(/(.+),(\w\w),.*/g)]
// layer1 = original.replaceAll(/(.+),(\w\w),.*/g, ["$1", "$2"])
// layer2 = layer1.replaceAll('""','"')

dictionary = {}
layer1.forEach(entry => {
    dictionary[entry[2]] = entry[1].replaceAll('"','')
});
